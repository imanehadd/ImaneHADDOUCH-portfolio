...
// Mobile menu toggle with overlay and animation - Performance optimized
function toggleMobileMenu() {
    const overlay = document.getElementById('mobile-menu-overlay');
    const panel = document.getElementById('mobile-menu-panel');
    if (!overlay || !panel) return;

    // Early setup of will-change to hint browser about upcoming animations
    panel.style.willChange = 'transform, opacity';
    
    if (overlay.classList.contains('hidden')) {
        // Batch DOM reads
        const previouslyFocused = document.activeElement;
        const firstFocusable = panel.querySelector('a,button,textarea,input,select');
        
        // Batch DOM writes
        requestAnimationFrame(() => {
            // Add contain: layout to prevent reflows outside the panel
            ...
            requestAnimationFrame(ariaUpdates);
            ...
            requestAnimationFrame(() => {
                ...
                // Apply focus and focus trap after animation starts
        ...
        requestAnimationFrame(() => {
            ...
            // Use RAF + timeout for smooth closing
            setTimeout(() => requestAnimationFrame(cleanup), 300);
        });
    }
}
